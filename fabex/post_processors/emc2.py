...
class Creator(iso.Creator):
    ...
    def BORE_FEED_OUT(self):
        return "G85"

    def BORE_SPINDLE_STOP_RAPID_OUT(self):
        return "G86"

    def BORE_DWELL_FEED_OUT(self, format, dwell):
    ...
    def start_CRC(self, left=True, radius=0.0):
        ...
        if left:
            self.write(
                ...
                + "\t (start left cutter radius compensation)\n"
            )
        else:
            self.write(
                ("G42" + self.SPACE() + "D%i") % self.t
                + "\t (start right cutter radius compensation)\n"
            )
